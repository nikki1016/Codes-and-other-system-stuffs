...
int main(){
	int graph[MAX][MAX] , pathlength[MAX], pred[MAX], state[MAX],pathl_f[MAX][MAX],pred_f[MAX][MAX];
	int v,s,des,d_un,i,j;
	scanf("%d%d",&v,&d_un);
	for(i = 0; i<v; i++){
...
void floyd(int arr[][MAX],int v,int pathlength[][MAX],int predecessor[][MAX]){
	...
	display(dis,v);
	printf("predecessor matrix of shortest path is \n");
	display(predecessor,v);
	//printf("\n");
}
